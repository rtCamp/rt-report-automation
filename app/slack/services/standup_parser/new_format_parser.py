...
class NewFormatParser(BaseParser):
	...
	def _normalize_for_matching(self, text: str) -> str:
		...
		# Remove markdown headers (## prefix) for new format
	...
	def _is_new_format_message(self, message: dict) -> bool:
	...
	def _message_has_questions(self, message: dict) -> bool:
		"""Check if a message contains any questions (new format).
		...
		"""
		return self._is_new_format_message(message)
	...
	def _parse_message_text(self, text: str) -> dict[str, list[str]]:
		"""Parse message text into structured Q&A pairs (new format).
		...
		# Skip the new format header line
		lines = normalized_text.split("\n")
		start_idx = 0
		if lines and lines[0].strip().lower().startswith(
			NEW_FORMAT_IDENTIFIER.lower(),
		):
			start_idx = 1
